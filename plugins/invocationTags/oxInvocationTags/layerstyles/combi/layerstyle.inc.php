...
function MAX_layerPutJs($output, $uniqid)
{
  ...
  // contruct deliverUrl for iFrame here

 //  $contenturl = MAX_commonGetDeliveryUrl($conf['file']['content']) . "?bannerid=" . $output['bannerid'] . "&zoneid=" . $output['zoneid'] . "&source=" . $GLOBALS['source'] . "&ct0=" . $GLOBALS['ct0'] . ";

?>

// alert('<?php echo $output['width'] ?>');
...
//   right_top_div.appendChild(close_link);
              ...
              //     c = c.style;
                 ...
                 switch(what)
                 {
                   case 'close':
                       ...
                       // log_imp.style.width = '0pxlog_imp
                       // ';.src = '<?php echo $GLOBALS['logUrl_Layer'] ?> ';
                   ...
                   case 'open':
                       ...
                       setTimeout("right_top_div.appendChild(close_link);", 2500);
                        // 3000("adooza_big_div_fadeIn()", 1000);
                           // alert('open');
                       // call popup now to avoid popup blocker
                       
                         // gobals source variabe from line 3574 delivery file

                         var script_tag = document.createElement('script');
                            script_tag.setAttribute("type","text/javascript");
                            script_tag.setAttribute("src", "http://dev.adooza.com/adserver/www/delivery/apu.php?&zoneid=<?php echo $GLOBALS['pop'] ?>&source=<?php echo $GLOBALS['source'] ?>&target=_blank&popunder=1&combi=1");
