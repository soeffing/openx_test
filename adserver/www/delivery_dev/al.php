...
// function to extract the pop ad id for the combi ad
MAX_commonRegisterGlobalsArray(array('pop'));
...
if ($limitations['compatible']) {
   ...
   // get the content url for src attribute of the iframe via the ac.php file <- capping included

   $GLOBALS['contenturl'] = MAX_commonGetDeliveryUrl($conf['file']['content']) . "?bannerid={$output['bannerid']}&zoneid={$zoneid}&source=".urlencode($source)."&ct0={$GLOBALS['ct0']}";
